...
public class TestClassExecutionHelper {
    ...
    public static List<TestOutcome> runTestClass(
            final Class<?> clazz,
            final EnvironmentConfigurationOverride... environmentConfigurationOverrides) {
        ...
        Stream.of(environmentConfigurationOverrides)
                .forEach(it -> mockEnvironmentVariable.setProperty(it.getKey(), it.getValue()));
        final Future<List<TestOutcome>> future = executorService
                .submit(new TestClassExecutionCallable(clazz, mockEnvironmentVariable));
    ...
    private static class TestClassExecutionCallable implements Callable<List<TestOutcome>> {
        ...
        private List<TestOutcome> executeTestClass() {
            ...
            EngineTestKit.engine("junit-jupiter").selectors(DiscoverySelectors.selectClass(clazz)).execute();
